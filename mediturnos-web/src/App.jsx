...
// ── EXPORTAR CSV ─────────────────────────────────────────────────────────────
function generarCSV(turnos, opciones) {
  const cols = []
  ...
  if (opciones.fechaNacimiento) cols.push(['Fecha_nac',   t => t.fecha_nacimiento || ''])
  if (opciones.dni)             cols.push(['DNI',          t => t.dni || ''])
  if (opciones.obraSocial)      cols.push(['Obra_social',  t => t.obra_social || ''])
  ...
  if (opciones.fecha)           cols.push(['Fecha',        t => t.fecha || ''])
  if (opciones.motivoConsulta)  cols.push(['Motivo_consulta',   t => t.motivo_consulta || ''])
  if (opciones.motivoCancelacion) cols.push(['Motivo_cancelacion', t => t.motivo_cancelacion || ''])
  if (opciones.creadoPor)       cols.push(['Creado_por',  t => t.creado_por || ''])
    ...
    <div className="app">
      ...
      {pantalla === 'panel' && sesion && (
        <Panel
          ...
          onExportarCSV={() => setModal(<ExportarCSVModal turnos={todosLosTurnos} />)}
        />
      )}
    </div>
  )
...
function CalendarioDisponibilidad({ todosLosTurnos, onNuevoTurno, onTurnoCalendario }) {
                ...
                return (
                  <div key={hora}
                    ...
                    onClick={() => {
                      ...
                      if (ocupado) {
                        const turnoEnCelda = todosLosTurnos.find(t =>
                          t.especialidad?.toLowerCase() === espSel.toLowerCase() &&
...
// ── EXPORTAR CSV MODAL ────────────────────────────────────────────────────────
function ExportarCSVModal({ turnos }) {
  ...
  const exportar = () => {
    ...
    const csv = generarCSV(filtrados, opc)
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' })
    const url  = URL.createObjectURL(blob)
    const a    = document.createElement('a')
    a.href = url
    a.download = `mediturnos_estadistica_${new Date().toISOString().slice(0,10)}.csv`
  ...
  return (
    <div>
      <h2 className="form-titulo">Exportar datos para Estadistica</h2>
